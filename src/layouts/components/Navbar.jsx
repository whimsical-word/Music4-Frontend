...
const Navbar = () => {
  ...
  const { userId, username, name, role, img, logout, isAuthenticated } =
    useAuthStore();
  const id = userId;
  ...
  useEffect(() => {
    ...
    const eventSource = new EventSource(
      `http://localhost:8080/api/notifications/subscribe/${id}`,
    );
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((noti) => (
                      ...
                      >
                        <button
                          onClick={(e) =>
                            handleDeleteNoti(e, noti.id, noti.isRead)
                          }
                        ...
                        <p
                          className={`text-xs text-white leading-relaxed pr-5 ${!noti.isRead ? "font-semibold" : "font-normal"}`}
                        >
                          {noti.content}
                        </p>
        ...
        <div className="relative" ref={dropdownRef}>
          {isAuthenticated ? (
            ...
            >
              <MusicImage
                src={
                  img && img !== "null"
                    ? img
                    : `https://ui-avatars.com/api/?name=${username || "U"}&background=0D8BFF&color=fff&bold=true&size=128`
                }
                alt="User Avatar"
            ...
            >
              ...
              <ChevronDown
                size={14}
                className={`text-[#a7a7a7] transition-transform duration-200 ${dropdownOpen ? "rotate-180" : ""}`}
              />
            </button>
          )}
            ...
            <div className="absolute right-0 mt-2 w-52 bg-[#282828] border border-[#3e3e3e] rounded-lg shadow-2xl py-1 z-50 animate-fadeIn">
              {isAuthenticated ? (
                ...
                >
                  <LogOut
                    size={16}
                    className="text-blue-500 transform rotate-180"
                  />
                  <span>Login</span>
                </button>
              )}
