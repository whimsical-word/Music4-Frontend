...
const Sidebar = () => {
    ...
    const { playlists, fetchMyPlaylists, updatePlaylist, deletePlaylist } = usePlaylistStore(); // 🟢 Lấy thêm hàm sửa/xóa từ store
    ...
    const handleDelete = async (e, id, name) => {
        ...
        if (window.confirm(`⚠️ Bạn có chắc chắn muốn xóa playlist "${name}" không?`)) {
        ...
        <div className="w-64 bg-[#0d131a] p-6 flex flex-col h-full border-r border-white/[0.05] hidden md:flex font-sans text-slate-400">
            ...
            {role === 'admin' && (
                <div className="mt-8 pt-6 border-t border-white/[0.05] flex flex-col gap-2 font-medium">
                    ...
                    <button onClick={() => handleAdminTabChange('overview')} className={`flex items-center gap-4 py-2 px-3 rounded-xl text-sm font-semibold transition-all w-full text-left bg-transparent border-none outline-none cursor-pointer ${isAdminPage && currentTab === 'overview' ? 'bg-white/[0.06] text-white border-l-2 border-sky-400 rounded-l-none pl-3' : 'hover:text-white hover:bg-white/[0.03]'}`}><LayoutDashboard size={18} /> Tổng quan</button>
                    <button onClick={() => handleAdminTabChange('users')} className={`flex items-center gap-4 py-2 px-3 rounded-xl text-sm font-semibold transition-all w-full text-left bg-transparent border-none outline-none cursor-pointer ${isAdminPage && currentTab === 'users' ? 'bg-white/[0.06] text-white border-l-2 border-sky-400 rounded-l-none pl-3' : 'hover:text-white hover:bg-white/[0.03]'}`}><Users size={18} /> Người dùng</button>
                    <button onClick={() => handleAdminTabChange('artists')} className={`flex items-center gap-4 py-2 px-3 rounded-xl text-sm font-semibold transition-all w-full text-left bg-transparent border-none outline-none cursor-pointer ${isAdminPage && currentTab === 'artists' ? 'bg-white/[0.06] text-white border-l-2 border-sky-400 rounded-l-none pl-3' : 'hover:text-white hover:bg-white/[0.03]'}`}><Mic2 size={18} /> Nghệ sĩ</button>
                    <button onClick={() => handleAdminTabChange('categories')} className={`flex items-center gap-4 py-2 px-3 rounded-xl text-sm font-semibold transition-all w-full text-left bg-transparent border-none outline-none cursor-pointer ${isAdminPage && currentTab === 'categories' ? 'bg-white/[0.06] text-white border-l-2 border-sky-400 rounded-l-none pl-3' : 'hover:text-white hover:bg-white/[0.03]'}`}><LayoutGrid size={18} /> Thể loại</button>
                </div>
            )}
            ...
            {role !== 'artist' && role !== 'admin' && (
                <div className="mt-8 pt-6 border-t border-white/[0.05] flex flex-col gap-1 font-medium">
                    ...
                    <Link to="/favorites" className={`flex items-center gap-4 py-2 px-3 rounded-xl text-sm transition-all group ${location.pathname === '/favorites' ? 'bg-white/[0.06] text-white font-semibold' : 'hover:text-white hover:bg-white/[0.03]'}`}>
                        <div className="bg-white/[0.04] border border-white/[0.02] p-1.5 rounded-lg text-slate-300"><Heart size={18} /></div>
                        Bài hát đã thích
                    </Link>
                    ...
                    >
                        <span className="flex items-center gap-4">
                            <div className="bg-white/[0.04] border border-white/[0.02] p-1.5 rounded-lg text-slate-300">
                                <Music2 size={18} className={isDropdownOpen ? "text-sky-400" : "text-slate-300"} />
                            </div>
                            <span className={isDropdownOpen ? "text-sky-400 font-semibold" : ""}>Playlist của tôi</span>
                        </span>
                    ...
                    {isDropdownOpen && (
                        <div className="mt-1 flex flex-col gap-1 transition-all duration-300">
                            ...
                            <button
                                onClick={() => userId ? setIsModalOpen(true) : alert("Vui lòng đăng nhập để tạo playlist!")}
                            ...
                            >
                                ...
                                <span className="font-semibold text-slate-400 hover:text-white">Tạo Playlist mới</span>
                            </button>

                            {/* Danh sách Playlist con có nút Ba chấm Tùy chọn */}
                            <div className="max-h-[260px] overflow-y-auto custom-scrollbar flex flex-col gap-1 mt-1" ref={menuRef}>
                                <ul className="flex flex-col gap-1 text-sm p-0 m-0 list-none">
                                    {playlists.length > 0 ? (
                                        playlists.map((pl) => {
                                            const isPlaylistActive = location.pathname === `/playlist/${pl.id}`;
                                                ...
                                                <li key={pl.id} className="w-full relative group/item">
                                                    ...
                                                    >
                                                        ...
                                                        <button
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                                e.stopPropagation();
                                                                setActiveMenuPlaylistId(isMenuOpen ? null : pl.id);
                                                            }}
                                                    ...
                                                    {isMenuOpen && (
                                                        <div className="absolute right-2 top-9 bg-[#161f2c] border border-white/[0.08] rounded-lg shadow-xl z-50 py-1 w-28 flex flex-col animate-in fade-in slide-in-from-top-1 duration-150">
                                                            ...
                                                            >
                                                                <Edit3 size={12} className="text-sky-400" /> Sửa tên
                                                            </button>
                                                            <button
                                        ...
                                        <li className="text-xs text-slate-600 italic py-2 px-3 pl-11">Chưa có danh sách phát</li>
                                    )}
                                </ul>
                            </div>
                        </div>
