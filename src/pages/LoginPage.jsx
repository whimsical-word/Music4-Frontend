...
const LoginPage = () => {
  ...
  const { loginSuccess, error, setAuthError, isLoading, setLoading } =
    useAuthStore();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (Array.isArray(rawRole)) {
        finalRole =
          rawRole[0]?.replace("ROLE_", "").toLowerCase() || "listener";
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-4 font-sans text-gray-100">
      <div className="w-full max-w-md bg-[#121212] border border-[#282828] p-8 rounded-2xl shadow-xl transition-all hover:border-[#3e3e3e]">
        <div className="flex flex-col items-center mb-10">
          ...
          <p className="text-xs text-[#a7a7a7] mt-1 uppercase tracking-widest">
            Hệ thống đăng nhập
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-xs font-semibold text-[#a7a7a7] uppercase tracking-wider mb-2">
              Tên tài khoản
            </label>
