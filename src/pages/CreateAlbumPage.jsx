...
const CreateAlbumPage = () => {
    ...
    const { userId, username } = useAuthStore(); // Lấy thêm username để so sánh tên nghệ sĩ
    ...
    // 🎯 Hàm helper kiểm tra xem 1 artist trong DB có phải là nghệ sĩ đang đăng nhập hay không
    const isCurrentArtist = (artist) => {
        return (
            Number(artist.id) === Number(userId) ||
            Number(artist.userId) === Number(userId) ||
            Number(artist.user?.id) === Number(userId) ||
            (artist.name && username && artist.name.trim().toLowerCase() === username.trim().toLowerCase())
        );
    };
    ...
    const filteredArtists = dbArtists.filter(artist =>
        ...
        !isCurrentArtist(artist) // Loại bỏ chính nghệ sĩ đang tạo album
    );
    ...
    const handleAlbumCoverChange = (e) => {
        ...
        setErrorMessage(""); // Clear error when selecting a new cover
    ...
    const handleMusicFilesSelect = (e) => {
        ...
        setErrorMessage(""); // Clear error when selecting new tracks
    ...
    const handlePublishAlbum = async (e) => {
        ...
        // 🔴 Validation 1: Tên album & Ảnh bìa
        if (!albumName.trim() || !albumCoverFile) {
            setErrorMessage("Vui lòng điền tên Album và chọn ảnh bìa!");
            return;
        }

        // 🔴 Validation 2: Số lượng bài hát
        if (tracks.length === 0) {
            setErrorMessage("Album phải có ít nhất một bài hát!");
            return;
        }

        // 🔴 Validation 3: Tiêu đề từng bài hát
        const emptyTitleTrack = tracks.find((track) => !track.title || track.title.trim() === "");
        if (emptyTitleTrack) {
            setErrorMessage(`Vui lòng nhập tên cho bài hát có tệp: ${emptyTitleTrack.fileName}!`);
            return;
        }

        // 🔴 Validation 4: Thể loại nhạc từng bài
        const invalidTrack = tracks.find((track) => track.categoryIds.length === 0);
        if (invalidTrack) {
            setErrorMessage(`Bài hát "${invalidTrack.title}" chưa được chọn thể loại (Category). Vui lòng chọn ít nhất 1 thể loại!`);
            return;
        }
        ...
        try {
            ...
            // 🔴 Bắt và phân tích phản hồi lỗi từ Backend
            let apiMessage = "Phát hành thất bại, vui lòng kiểm tra lại kết nối mạng hệ thống nha!";
            if (error.response && error.response.data) {
                const data = error.response.data;
                if (typeof data === "object" && !data.message) {
                    apiMessage = Object.values(data).join(" | ");
    ...
    return (
        <div className="bg-[#0d131a] min-h-screen text-slate-100 p-8 font-sans">
            <div className="max-w-6xl mx-auto">
                ...
                {errorMessage && (
                    <div className="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/30 flex items-start gap-3 text-red-400 text-sm shadow-lg">
                        <AlertCircle size={18} className="shrink-0 mt-0.5 text-red-400" />
                        <div className="flex-1">
                            <strong className="font-semibold block mb-0.5">Không thể phát hành:</strong>
                                                ...
                                                <div className="absolute z-50 w-full bg-[#131c26] border border-white/[0.1] mt-1 rounded-lg max-h-40 overflow-y-auto shadow-2xl">
                                                    {dbArtists
                                                        .filter(a =>
                                                            ...
                                                            !isCurrentArtist(a) // 🎯 KHÔNG hiển thị chính mình trong danh sách Feat từng track
                                                        )
                                                        .map(artist => (
                                                            <div
                                                                key={artist.id}
