...
const AdminDashboardPage = () => {
    ...
    const handleSaveCategory = async () => {
        ...
        if (artistsRes.status === "fulfilled") {
          ...
          // Bóc tách mảng linh hoạt giống như User
          const artistList =
            artistData?.content || artistData?.data || artistData || [];
          setArtists(Array.isArray(artistList) ? artistList : []);

          // Lấy totalPages một cách an toàn
          setArtistTotalPages(
            artistData?.page?.totalPages || artistData?.totalPages || 0,
          );
        }
        ...
        if (statsRes.status === "fulfilled")
          setCategoryStats(statsRes.value.data);
        if (topArtistsRes.status === "fulfilled")
          setTopArtists(topArtistsRes.value.data);
        if (topTracksRes.status === "fulfilled")
          setTopTracks(topTracksRes.value.data);
        if (userCountRes.status === "fulfilled")
          setUserCount(userCountRes.value.data || 0);
      } catch (error) {
        console.error("Lỗi lấy dữ liệu tổng hợp admin:", error);
      } finally {
        setIsLoading(false);
  ...
  const handleDeleteUser = async (id) => {
    if (
      window.confirm(
        "Bạn có chắc chắn muốn xóa người dùng này? (Dữ liệu liên quan sẽ được xử lý an toàn)",
      )
    ) {
      try {
  ...
  const handleDeleteArtist = async (id) => {
    if (
      window.confirm(
        "Bạn có chắc chắn muốn xóa nghệ sĩ này? (Dữ liệu liên quan sẽ được xử lý an toàn)",
      )
    ) {
      try {
              ...
              <div>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      ...
                      />
                      <span>
                        {isCleaning ? "Đang dọn dẹp..." : "Dọn Dẹp Lịch Sử"}
                      </span>
                    </button>
                    <button
                    ...
                    >
                      ...
                      />
                      <span>
                        {isSyncing ? "Đang đồng bộ..." : "Đồng Bộ Dữ Liệu AI"}
                      </span>
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-[#181818] p-6 rounded-2xl border border-[#282828] shadow-lg flex flex-col h-[400px]">
                    ...
                    <p className="text-xs text-[#a7a7a7] mb-4">
                      Mức độ phân bổ bài hát trên hệ thống
                    </p>
                      ...
                      {topTracks.map((track, idx) => (
                        ...
                        >
                          <div className="flex items-center gap-4 overflow-hidden">
                            <span
                              className={`font-black text-lg w-4 text-center ${idx === 0 ? "text-yellow-500" : idx === 1 ? "text-gray-400" : idx === 2 ? "text-amber-700" : "text-[#a7a7a7]"}`}
                            >
                              {idx + 1}
                            </span>
                ...
                <div className="bg-[#181818] p-6 rounded-2xl border border-[#282828] shadow-lg">
                  ...
                  <p className="text-xs text-[#a7a7a7] mb-6">
                    Đánh giá theo tương tác tổng hợp (Followers + Favorites)
                  </p>
                        ...
                        <div className="flex gap-4 mt-3 pt-3 border-t border-[#333] w-full justify-center">
                          <div>
                            ...
                            <p className="text-sm font-bold text-emerald-500">
                              {(
                                artist.followers + artist.favorites
                              ).toLocaleString()}
                            </p>
                          </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-[#282828] text-sm">
                      {users.map((u) => (
                        <tr
                          key={u.id}
                          className="hover:bg-[#222222]/50 transition-colors"
                        >
