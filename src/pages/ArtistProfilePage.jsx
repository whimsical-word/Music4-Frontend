...
const ArtistProfilePage = () => {
    ...
    useEffect(() => {
        const fetchArtistData = async () => {
            ...
            try {
                ...
                let albumList = [];
                if (albumsRes.data) {
                    albumList = albumsRes.data.content || albumsRes.data.data || albumsRes.data;
                } else {
                ...
                let trackList = [];
                if (tracksRes.data) {
                    trackList = tracksRes.data.content || tracksRes.data.data || tracksRes.data;
                } else {
    ...
    const handleDeleteTrack = async (trackId, trackName) => {
        ...
        try {
            ...
            if (artistInfo) {
                setArtistInfo(prev => ({ ...prev, trackTotal: Math.max(0, prev.trackTotal - 1) }));
            }
        } catch (error) {
    ...
    if (isLoading) return <div className="bg-[#0a0f14] min-h-screen p-20 flex items-center justify-center"><div className="w-10 h-10 border-4 border-slate-700 border-t-sky-500 rounded-full animate-spin"></div></div>;
    if (!artistInfo) return <div className="bg-[#0a0f14] min-h-screen p-20 text-center text-slate-400">Không tìm thấy thông tin nghệ sĩ.</div>;
    ...
    return (
        <div className="bg-gradient-to-b from-[#07192c] via-[#0d131a] to-[#0a0f14] min-h-screen text-white font-sans pb-32 relative">
            ...
            <div className="relative h-[40vh] min-h-[350px] flex items-end p-8">
                ...
                <div className="relative z-10 flex items-center gap-6">
                    <div className="w-48 h-48 rounded-full shadow-[0_12px_32px_rgba(0,0,0,0.6)] overflow-hidden border-4 border-white/[0.06] relative group bg-[#16222f]">
                        <MusicImage src={artistInfo.img} type="artist" className="w-full h-full object-cover" />
                        ...
                        {isOwner && (
                            ...
                            >
                                ...
                                <span className="text-white text-xs font-bold tracking-wide">Sửa hồ sơ</span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col">
                        ...
                        <p className="text-sm text-slate-400 font-medium mt-2">
                            {artistInfo.trackTotal || 0} Bài hát • {artistInfo.albumTotal || 0} Album
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="p-8 relative z-10 space-y-12">
                ...
                {isOwner && stats && (
                    <section className="bg-[#111a24]/40 border border-white/[0.04] p-6 rounded-2xl shadow-xl backdrop-blur-sm">
                        <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-slate-200">
                            <Activity className="text-sky-400" size={20} /> Tổng quan dữ liệu của bạn
                        </h2>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="bg-white/[0.03] border border-white/[0.02] p-5 rounded-xl">
                                <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Tổng lượt nghe</p>
                                <p className="text-3xl font-black text-white">{stats.totalViews?.toLocaleString() || 0}</p>
                            </div>
                            <div className="bg-white/[0.03] border border-white/[0.02] p-5 rounded-xl">
                                <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Tổng người theo dõi</p>
                                <p className="text-3xl font-black text-white">{stats.totalFollowers?.toLocaleString() || 0}</p>
                            </div>
                            <div className="bg-white/[0.03] border border-white/[0.02] p-5 rounded-xl">
                                <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Lượt thả tim</p>
                ...
                <div className="px-4 py-1 flex items-center gap-270">
                    ...
                    {isOwner && stats && (
                        <div className="mt-8 pt-6 border-t border-white/[0.05] flex flex-col gap-4 font-medium">
                            <button onClick={() => navigate('/studio/upload')} className="flex items-center gap-4 hover:text-sky-400 transition-all group w-full text-left bg-transparent border-none outline-none cursor-pointer p-0">
                                <div className="bg-white/[0.04] border border-white/[0.02] group-hover:bg-white/[0.08] group-hover:text-sky-400 p-1.5 rounded-lg text-slate-300 transition-colors"><Mic2 size={18} /></div>
                                Tạo Bài hát mới
                            </button>
                            <button onClick={() => navigate('/studio/album')} className="flex items-center gap-4 hover:text-sky-400 transition-all group w-full text-left bg-transparent border-none outline-none cursor-pointer p-0">
                                <div className="bg-white/[0.04] border border-white/[0.02] group-hover:bg-white/[0.08] group-hover:text-sky-400 p-1.5 rounded-lg text-slate-300 transition-colors"><Disc size={18} /></div>
                                Tạo Album mới
                            </button>
                        </div>
                    )}
                ...
                <section>
                    <h2 className="text-xl font-bold mb-6 text-slate-200">Bài hát đã phát hành</h2>
                    {tracks.length > 0 ? (
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-4 flex-1 min-w-0">
                                        ...
                                        <div className="truncate">
                                            ...
                                            <p className="text-xs text-slate-400 truncate">{artistInfo.name}</p>
                                        </div>
                                    </div>
                        ...
                        <p className="text-slate-400 text-sm bg-[#111a24]/20 p-4 rounded-xl border border-white/[0.04]">Nghệ sĩ này chưa đăng bài hát nào.</p>
                    )}
                </section>

                {/* 5. DANH SÁCH ALBUM CỦA NGHỆ SĨ */}
                <section>
                    <h2 className="text-xl font-bold mb-6 hover:text-sky-400 transition-colors cursor-pointer inline-block">Album</h2>
                    {albums.length > 0 ? (
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                            {albums.map((album) => (
                                <div key={album.id} onClick={() => navigate(`/albums/${album.id}`)} className="bg-[#111a24]/40 border border-white/[0.04] p-4 rounded-xl hover:bg-white/[0.06] transition-all duration-300 group cursor-pointer shadow-md">
                                    ...
                                    <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">{album.uploadDate ? new Date(album.uploadDate).getFullYear() : "----"} • Album</p>
                                </div>
                            ))}
                        </div>
                    ) : (
                        <p className="text-slate-400 text-sm bg-[#111a24]/20 p-4 rounded-xl border border-white/[0.04]">Nghệ sĩ này chưa phát hành album nào.</p>
                    )}
                </section>
            ...
            {isEditModalOpen && (
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] animate-fadeIn backdrop-blur-sm">
                    <div className="bg-[#0f1722] border border-white/[0.06] p-6 rounded-2xl w-[500px] shadow-2xl relative text-white">
                        <button onClick={() => setIsEditModalOpen(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white cursor-pointer bg-transparent border-none z-20 transition-colors">
                            <X size={22} />
                        </button>
                        <h2 className="text-xl font-bold mb-6 text-slate-100">Chi tiết hồ sơ Nghệ sĩ</h2>
                        ...
                        <form onSubmit={handleUpdateProfile} className="flex flex-col gap-6">
                            ...
                            <div className="relative h-32 w-full rounded-xl overflow-hidden bg-[#16222f] group shadow-inner border border-white/[0.04]">
                                {previewCover ? (
                                    <img src={previewCover} alt="Cover Preview" className="w-full h-full object-cover group-hover:brightness-50 transition-all" />
                                ) : (
                                    <MusicImage src={artistInfo.cover || artistInfo.img} type="artist" className="w-full h-full object-cover group-hover:brightness-50 transition-all" />
                                )}
                                ...
                                <div className="absolute top-2 left-2 bg-black/60 px-2 py-0.5 rounded text-[10px] text-slate-300 font-bold tracking-widest uppercase border border-white/[0.05]">ẢNH BÌA</div>
                            </div>
                            ...
                            <div className="flex justify-center -mt-16 relative z-10">
                                <label className="relative w-28 h-28 rounded-full cursor-pointer group shadow-2xl border-4 border-[#0f1722] hover:border-sky-500 transition-all bg-[#0a0f14]">
                                    {previewAvatar ? (
                                        <img src={previewAvatar} alt="Avatar Preview" className="w-full h-full object-cover rounded-full group-hover:brightness-50 transition-all" />
                                    ) : (
                            ...
                            {/* ĐỔI TÊN */}
                            <div>
                                <label className="text-xs font-bold text-slate-400 mb-2 block uppercase tracking-wider">Tên Nghệ sĩ</label>
                                <input
                                    type="text"
                                    value={editName}
