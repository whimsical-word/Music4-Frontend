...
// 🟢 Import Component xử lý ảnh thông minh của bạn vào
import MusicImage from '../layouts/components/MusicImage';

const ProfilePage = () => {
    const navigate = useNavigate();
    const { id, username, img, role } = useAuthStore();
    ...
    // MOCK DATA: Nghệ sĩ yêu thích (Khung giao diện tạm thời trong lúc chờ BE)
    ...
    useEffect(() => {
        const fetchMyPlaylists = async () => {
            try {
                // Gọi API lấy playlist của user đang đăng nhập
                const res = await axiosClient.get('/playlists/me');
    ...
    const handleUpdateProfile = async (e) => {
        ...
        try {
            ...
            if (selectedFile) {
                await axiosClient.post(`/images/users/${id}/avatar`, formData, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            }

            // TODO: Gọi thêm API Update Name nếu Backend hỗ trợ ở User Controller
        ...
        <div className="bg-[#121212] min-h-screen text-white font-sans pb-32">
            ...
            <div className="bg-gradient-to-b from-[#535353] to-[#121212] h-[340px] px-8 flex items-end pb-8">
                <div className="flex items-center gap-6">
                    ...
                    >
                        {/* Sử dụng MusicImage, truyền link UI Avatar nếu img trên DB rỗng */}
                        <MusicImage
                            src={img && img !== 'null' ? img : fallbackAvatar}
                            type="user"
                            alt="Profile"
                    ...
                    <div className="flex flex-col">
                        ...
                        <h1 className="text-5xl md:text-7xl font-black mb-4 tracking-tighter">{username}</h1>
